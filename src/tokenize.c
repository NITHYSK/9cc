...
Token *tokenize(char *p) {
	...
	while (*p) {
		...
		if ('a' <= *p && *p <= 'z' || *p == '_') {
			...
			cur->len = getIdentLength(&p);
			cur->idname = calloc(cur->len + 1, sizeof(char));
			snprintf(cur->idname, cur->len + 1, "%s", cur->str);
			dbg("idname	= %s", cur->idname);
			continue;
		}
		if (*p == '=') {
			dbg();
...
int getIdentLength(char **endptr) {
	char *p = *endptr;
	int len = 0;
	if (('a' <= *p && *p <= 'z') || ('A' <= *p && 'Z' <= *p) || *p == '_') {
		*endptr += 1;  // p+=1
		len = getIdentLength(endptr);
		return len + 1;
	}
	return len;
}
